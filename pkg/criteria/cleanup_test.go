...
func TestCleanUpJudge_filterByNamespace(t *testing.T) {
	createJudge := func(namespaces []string) CleanUpJudge {
		return CleanUpJudge{
			deniedNamespaces:  namespaces,
			gracePeriod:       0,
			notificationDelay: 0,
		}
	}
	...
	deploymentTest := []struct {
		...
		},
		{
			...
			Namespace:        "test",
			DeniedNamespaces: []string{"test"},
			Expected:         false,
		},
		{
			Name:             "Denied namespaces not included in cleanup",
			Namespace:        "guri",
			DeniedNamespaces: []string{"guri", "tor", "marianne"},
			Expected:         false,
		},
		{
			Name:             "Works when name is contained in denied namespace",
			Namespace:        "odd",
			DeniedNamespaces: []string{"oddrane"},
			Expected:         false,
		},
		{
			Name:             "Not working namespace",
			Namespace:        "notworking",
			DeniedNamespaces: []string{"allowed"},
			Expected:         true,
		},
		{
			Name:             "Empty denied namespaces",
			Namespace:        "test",
			DeniedNamespaces: []string{},
			Expected:         true,
		},
		{
			...
			DeniedNamespaces: []string{"babylon-test", "kuttl-test"},
			Expected:         false,
		},
	}
	...
	for _, tt := range deploymentTest {
		...
		t.Run(tt.Name, func(t *testing.T) {
			...
			judge := createJudge(tt.DeniedNamespaces)
			deployment := createDeployment(tt.Namespace, nil)
			actual := judge.filterByDeniedNamespace(&deployment)
...
func TestCleanUpJudge_Judge_not_allowed_namespace(t *testing.T) {
	...
	deployment := createDeployment("default", nil)
